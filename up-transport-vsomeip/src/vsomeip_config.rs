 ...
 ********************************************************************************/
 ...
 pub(crate) fn extract_application(file_path: &Path) -> Result<VsomeipApplicationConfig, UStatus> {
     let json_data = read_json_file(file_path).map_err(|e| {
         UStatus::fail_with_code(UCode::INVALID_ARGUMENT, format!("Failed to read JSON File: {e}"))
     })?;
     ...
     let app_config: VsomeipApplicationConfig = serde_json::from_value(applications[0].clone())
         .map_err(|e| {
             UStatus::fail_with_code(UCode::INVALID_ARGUMENT, format!(": {e}"))
         })?;
 ...
 pub(crate) fn extract_services(file_path: &Path) -> Result<Vec<ServiceConfig>, UStatus> {
     ...
     let service_configs: Vec<ServiceConfig> = serde_json::from_value(Value::Array(services.clone()))
         .map_err(|e| {
             UStatus::fail_with_code(UCode::INVALID_ARGUMENT, format!("Failed to change service: {e}"))
         })?;
 ...
 mod tests {
     ...
     fn test_deserialize_hex_u16() {
         ...
         assert!(deserialized.is_err(), "Expected an error due to string type");
     }
